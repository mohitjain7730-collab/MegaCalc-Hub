...
export const generateArticleSchema = (detail: ArticleDetail, slug: string) => {
  // Try to find author in DB to get correct Role for Schema if possible
  const dbAuthor = detail.author ? AUTHORS.find(a => a.name === detail.author) : null;
  
  const authorData = dbAuthor || (detail.author 
    ? { name: detail.author, role: "Financial Analyst" } 
...
export const generateFullArticleHTML = (detail: ArticleDetail): string => {
  ...
  // 1. If author name is provided in the detail object, try to find their full profile (Role/Bio) in our database
  if (detail.author) {
    const match = AUTHORS.find(a => a.name === detail.author);
    if (match) {
      // Exact match found in DB - use the DB entry to get the specific Role and Bio
      authorData = match;
    } else {
      // Manual author provided but not found in DB - Fallback to generic professional role
      authorData = { 
        name: detail.author, 
        bio: detail.authorBio || "Financial contributor and market analyst.", 
        role: "Financial Analyst" // Professional fallback instead of "Guest"
      };
    }
  } else {
    // 2. No author provided, auto-assign deterministically from DB
  ...
  // Construct Author Byline (Top) - Shows Name and Specific Role
  // This ensures the credential travels with the HTML content when copied
  const byline = `
    <div class="flex items-center text-sm text-gray-500 mb-8 pb-6 border-b border-gray-100 not-prose">
      ...
      <div>
        ...
        <span class="block text-xs font-medium text-brand-600 uppercase tracking-wide">${authorData.role}</span>
      </div>
    </div>
  `;
    ...
    <div class="mt-12 pt-8 border-t border-gray-200 bg-gray-50 rounded-xl p-6">
      ...
      <div class="flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-xs font-medium text-brand-600 uppercase tracking-wide mb-2">${authorData.role}</p>
