...
const teamMemberSchema = z.object({
    ...
    timezone: z.number(),
    startWork: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid time"),
    endWork: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid time"),
});
...
export default function TimeZoneOverlapCalculator() {
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            members: [
                { name: '', timezone: 0, startWork: '09:00', endWork: '17:00' },
                { name: '', timezone: 0, startWork: '09:00', endWork: '17:00' },
            ],
        },
    });
        ...
        <div className="space-y-8">
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            <div>
                                ...
                                <div className="space-y-4">
                                    {fields.map((field, index) => (
                                        <div key={field.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end border p-4 rounded-lg relative">
                                            ...
                                            <FormField control={form.control} name={`members.${index}.timezone`} render={({ field }) => (<FormItem><FormLabel>Timezone</FormLabel><Select onValueChange={(v) => field.onChange(parseFloat(v))} defaultValue={field.value?.toString()}><FormControl><SelectTrigger><SelectValue placeholder="Select timezone" /></SelectTrigger></FormControl><SelectContent>{TIMEZONE_OFFSETS.map(tz => (<SelectItem key={tz.name} value={tz.offset.toString()}>{tz.name}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                                ...
                                <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => append({ name: '', timezone: 0, startWork: '09:00', endWork: '17:00' })}><PlusCircle className="mr-2 h-4 w-4" /> Add Member</Button>
                            </div>
            ...
            <div className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent><ul className="list-disc pl-5 text-sm text-primary"><li><Link href="/category/employment/shift-rotation-calculator" className="hover:underline">Shift Rotation Calculator</Link></li></ul></CardContent>
                </Card>
