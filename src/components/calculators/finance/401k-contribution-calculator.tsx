...
import { Landmark, DollarSign, TrendingUp, Calendar, Target, Info, AlertCircle, Building, Users, FunctionSquare, Shield, Check, ArrowRight } from 'lucide-react';
...
export default function FourOhOneKCalculator() {
  ...
  const getRecommendations = (isMaxingOut: boolean, contributionPercentage: number, employerMatchLimit: number) => {
    const recs: { title: string; description: string; action?: string }[] = [];

    // Check if getting full match (approximate check based on contribution vs limit)
    if (contributionPercentage < employerMatchLimit) {
      recs.push({
        title: "Maximize Employer Match",
        description: "You might be leaving free money on the table if you contribute less than the match limit.",
        action: `Increase contribution to at least ${employerMatchLimit}%.`
      });
    } else {
      recs.push({
        title: "Full Match Captured",
        description: "Great job! You are taking full advantage of your employer's matching contributions.",
        action: "Keep this up as a baseline."
      });
    }

    if (isMaxingOut) {
      recs.push({
        title: "Maxing Out Limits",
        description: "You are hitting the annual IRS contribution limit.",
        action: "Consider IRAs or HSAs for additional tax-advantaged savings."
      });
    } else {
      recs.push({
        title: "Room to Contribute More",
        description: "You are below the annual contribution limit.",
        action: "Try the 'Auto-Escalation' method: increase by 1% every year."
      });
    }

    if (contributionPercentage + (contributionPercentage >= employerMatchLimit ? employerMatchLimit : 0) < 15) {
      recs.push({
        title: "Aim for 15%",
        description: "Experts often recommend saving 15% of income for retirement (including match).",
        action: "Review your budget to gradually handle higher contributions."
      });
    }
  ...
  const onSubmit = (values: FormValues) => {
    ...
    if (chartData.length > 0) {
      ...
      setResult({
        ...
        isMaxingOut,
        recommendations: getRecommendations(isMaxingOut, contributionPercentage, employerMatchLimit)
      });
    }
  };
    ...
    <div className="space-y-8">
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-muted rounded-lg overflow-x-auto">
              <p className="font-mono text-sm text-center">
                FV = Balance*(1+r)^n + (C + M)*[((1+r)^n - 1)/r]
              </p>
            </div>
            <div className="text-sm text-muted-foreground grid grid-cols-1 md:grid-cols-2 gap-4">
              <ul className="space-y-1">
                ...
                <li><span className="font-semibold">Balance</span> = Current Balance</li>
                <li><span className="font-semibold">r</span> = Monthly Interest Rate</li>
              </ul>
              <ul className="space-y-1">
                <li><span className="font-semibold">n</span> = Months until Retirement</li>
                <li><span className="font-semibold">C</span> = Monthly Contribution</li>
                <li><span className="font-semibold">M</span> = Monthly Employer Match</li>
              </ul>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          <p>This tool projects retirement savings based on 401(k) contributions, employer match, and compound growth.</p>
          <p>Recommendations, charts, formulas, guide content, and related tools provide comprehensive insights for financial planning.</p>
          <p>Use maximum contributions and full employer matching to optimize long-term wealth accumulation.</p>
        </CardContent>
      </Card>
    </div >
