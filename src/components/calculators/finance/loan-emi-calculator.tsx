...
import { HandCoins, DollarSign, TrendingUp, Calendar, Target, Info, AlertCircle, CreditCard, Home, Car, Activity, Shield } from 'lucide-react';
...
export default function LoanEmiCalculator() {
  ...
  const calculateEmi = (loanAmount: number, annualInterestRate: number, loanTenureYears: number): CalculationResult => {
    const P = loanAmount;
  ...
  const recommendationItems = result
    ? [
      `Your monthly EMI of $${result.emi.toLocaleString(undefined, { maximumFractionDigits: 0 })} requires consistent cash flow planning.`,
      result.interestPercentage > 50
        ? 'Interest makes up more than 50% of your total payment. Consider a shorter tenure or prepayments to save money.'
        : 'Your principal repayment is healthy. Stick to the schedule to build equity faster.',
      result.totalInterest > result.totalPayment * 0.2
        ? 'You are paying significant interest. Check if you can refinance for a lower rate.'
        : 'Your loan terms seem favorable with a reasonable interest burden.',
      'Consider aligning your EMI date with your salary credit date to avoid missed payments.',
    ]
    : [];
  ...
  const actionPlanItems = result
    ? [
      {
        label: 'Automate',
        detail: 'Set up auto-debit for your EMI to maintain a good credit score and avoid penalties.',
      },
      {
        label: 'Prepay',
        detail: 'Any bonus or surplus income should go towards principal prepayment to drastically reduce interest.',
      },
      {
        label: 'Protect',
        detail: 'Ensure you have loan insurance or sufficient life cover to protect your family from this liability.',
      },
      {
        label: 'Review',
        detail: 'Check interest rates annually. If market rates drop by 0.5-1%, consider refinancing.',
      },
    ]
    : [];
  ...
  const onSubmit = (values: FormValues) => {
    ...
    if (supportsWorker) {
      ...
      `;
        ...
        worker.onerror = () => {
          // Fallback to main-thread calculation if the worker fails for any reason (e.g., CSP)
          setResult(calculateEmi(loanAmount, annualInterestRate, loanTenureYears));
          worker.terminate();
          URL.revokeObjectURL(workerUrl);
        };
        ...
        console.error('Web Worker creation failed, falling back to main thread:', error);
        setResult(calculateEmi(loanAmount, annualInterestRate, loanTenureYears));
        return;
      }
    }

    // Fallback on main thread if Web Workers are not available or fail
    setResult(calculateEmi(loanAmount, annualInterestRate, loanTenureYears));
  };
      ...
      <div className="space-y-6">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  {actionPlanItems.map((step, index) => (
                    <li key={index} className="flex gap-2">
                      <span className="font-semibold text-primary min-w-[70px]">{step.label}:</span>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-muted/50 rounded-lg">
              <p className="font-mono text-center text-lg md:text-xl font-bold text-primary">
                E = [P × r × (1 + r)ⁿ] / [(1 + r)ⁿ - 1]
              </p>
            </div>
            <div className="text-sm text-muted-foreground space-y-2">
              ...
              <p className="mt-2">
                This formula calculates the fixed monthly payment needed to pay off both the principal and the accrued interest over the specified period.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>This tool models loan amortization using principal, interest rate, and tenure to show EMI, total interest, and total payment.</p>
            ...
            <p>Use prepayment strategies and tenure adjustments to optimize total interest outflow effectively.</p>
          </CardContent>
        </Card>
      </div>
    </div>
