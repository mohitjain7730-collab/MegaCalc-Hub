...
import { PiggyBank, DollarSign, TrendingUp, Calendar, Target, Info, AlertCircle, Landmark, Building, FunctionSquare, Shield, Check, ArrowRight } from 'lucide-react';
...
export default function CompoundInterestCalculator() {
  ...
  const getRecommendations = (yearsToDouble: number, interestPercentage: number, annualRate: number) => {
    const recs: { title: string; description: string; action?: string }[] = [];

    if (yearsToDouble < 8) {
      recs.push({
        title: "High Growth Trajectory",
        description: `Your money is doubling every ${yearsToDouble.toFixed(1)} years, indicating strong returns.`,
        action: "Consider tax-advantaged accounts to protect these gains."
      });
    } else if (yearsToDouble < 12) {
      recs.push({
        title: "Moderate Growth",
        description: "Your investment is growing steadily. Time is your best ally here.",
        action: "Try to increase your principal or monthly contributions."
      });
    } else {
      recs.push({
        title: "Conservative Growth",
        description: "Growth is slower. Ensure this aligns with your risk tolerance and goals.",
        action: "Compare this rate with other investment vehicles."
      });
    }
    ...
    if (interestPercentage > 100) {
      recs.push({
        ...
        description: "Your interest earned now exceeds your initial principal!",
        action: "Reinvest dividends/interest to keep the momentum."
      });
    } else {
      recs.push({
        title: "Early Stages",
        description: "You're in the accumulation phase. Interest is starting to build up.",
        action: "Stay consistent and let time work for you."
      });
    }
  ...
  const onSubmit = (values: FormValues) => {
    ...
    setResult({
      ...
      recommendations: getRecommendations(yearsToDouble, interestPercentage, annualRate)
    });
  };
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-sm text-muted-foreground grid grid-cols-1 md:grid-cols-2 gap-4">
            <ul className="space-y-1">
              <li><span className="font-semibold">A</span> = Final Amount</li>
              <li><span className="font-semibold">P</span> = Principal Investment</li>
              <li><span className="font-semibold">r</span> = Annual Interest Rate (decimal)</li>
            </ul>
            <ul className="space-y-1">
              <li><span className="font-semibold">n</span> = Compounding Frequency (per year)</li>
              <li><span className="font-semibold">t</span> = Number of Years</li>
            </ul>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>This tool calculates the future value of an investment using the compound interest formula A = P(1 + r/n)^(nt).</p>
            <p>Recommendations, growth charts, formulas, guide content, and related tools provide comprehensive insights for financial planning.</p>
            <p>Use higher compounding frequencies and longer time horizons to maximize exponential growth.</p>
          </CardContent>
        </Card>
      </div >
    </div >
