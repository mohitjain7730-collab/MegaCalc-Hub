...
import { Trophy, TrendingUp, AlertCircle, Target, Calculator, BarChart3, Shield, Info, CheckCircle2, TrendingDown, AlertTriangle, Percent } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

const formSchema = z.object({
    // Match situation
    runsNeeded: z.number().min(0, 'Runs needed must be positive or zero'),
    ballsRemaining: z.number().min(1, 'Balls remaining must be at least 1').max(600, 'Max 600 balls possible'),
    wicketsInHand: z.number().min(0).max(10, 'Wickets must be between 0 and 10'),
    currentRunRate: z.number().min(0, 'Current run rate must be positive or zero'),
    requiredRunRate: z.number().min(0, 'Required run rate must be positive or zero'),
    // Match format
...
export default function CricketWinProbabilityCalculatorInteractive() {
