...
export function ThemeToggle() {
    ...
    // Provide default values to avoid hydration mismatch or undefined states
    const [isCustomizing, setIsCustomizing] = React.useState(false);
    const [customForeground, setCustomForeground] = React.useState<string | null>(null);
    const [customBackground, setCustomBackground] = React.useState<string | null>(null);
    const [mounted, setMounted] = React.useState(false);

    // Ensure component only runs logic after mount to avoid hydration errors
    React.useEffect(() => {
        setMounted(true);
        setCustomForeground(localStorage.getItem("custom-foreground"));
        setCustomBackground(localStorage.getItem("custom-background"));
    }, []);
    ...
    React.useEffect(() => {
        if (mounted && customForeground && customBackground) {
            applyCustomColors(customForeground, customBackground);
        }
    }, [mounted, customForeground, customBackground]);
                ...
                <DropdownMenuContent align="end">
                    <DropdownMenuItem
                        onClick={() => {
                            ...
                            // Avoid full reload if possible, but keep if user intent is hard reset
                            // window.location.reload(); 
                            // Better to just let state update naturally, but preserving original reload behavior if deemed necessary
                            // The user had verify logic: window.location.reload()
                            // I will keep it as it resets CSS variables effectively
                            window.location.reload();
                        }}
                    >
                        Light
                            ...
                            {foregroundColors.map((color) => (
                                <Button
                                    ...
                                    onClick={() =>
                                        applyCustomColors(
                                            color.value,
                                            customBackground || backgroundColors[0].value
                                        )
                                    }
                                >
                                    <div
                        ...
                        <div className="flex flex-wrap gap-2">
                            {backgroundColors.map((color) => (
                                <Button
                                    ...
                                    onClick={() =>
                                        applyCustomColors(
                                            customForeground || foregroundColors[0].value,
                                            color.value
                                        )
                                    }
                                >
